...
export const metadata: Metadata = {
  title: "PyraRides Academy — Horse Riding Training in Egypt",
  description:
...
export default async function TrainingPage() {
  const academies = await prisma.academy.findMany({
    ...
    include: {
      captain: {
        select: { fullName: true, profileImageUrl: true, bio: true },
      },
      programs: {
        where: { isActive: true },
      ...
      <section className="relative h-[70vh] min-h-[500px] w-full overflow-hidden bg-black">
        ...
        <div className="relative z-10 flex h-full flex-col items-center justify-center text-center px-4">
          ...
          <p className="text-white/60 max-w-xl text-base md:text-lg font-light">
            Master the art of riding under the guidance of certified captains.
            Multiple academies across Egypt.
          </p>
            ...
            <div className="container mx-auto px-4 max-w-7xl">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8">
                {academies.map((academy) => (
                  <div
                    key={academy.id}
                    className="group border border-white/10 rounded-2xl overflow-hidden bg-white/[0.02] hover:bg-white/[0.04] transition-all duration-500"
                  >
                    {/* Academy Image */}
                    <div className="relative aspect-[16/9] overflow-hidden">
                      ...
                      <div className="absolute bottom-4 left-4">
                        <span className="bg-[#D4AF37]/20 backdrop-blur-sm border border-[#D4AF37]/30 text-[#D4AF37] text-[9px] uppercase tracking-[0.2em] px-3 py-1 rounded-full">
                          {academy.location}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="p-6">
                      ...
                      <div className="flex items-center gap-3 mb-6 pb-6 border-b border-white/10">
                        <div className="relative w-10 h-10 rounded-full overflow-hidden border border-white/20">
                          {academy.captain.profileImageUrl ? (
                            <Image
                              src={academy.captain.profileImageUrl}
                              alt={academy.captain.fullName || "Captain"}
                              fill
                        ...
                        <div>
                          <p className="text-white text-sm font-medium">
                            {academy.captain.fullName || "Captain"}
                          </p>
                      ...
                      <div className="space-y-3">
                        {academy.programs.map((program) => (
                          <div
                            key={program.id}
                            className="flex items-center justify-between py-3 px-4 rounded-xl bg-white/[0.03] border border-white/5 hover:border-[#D4AF37]/20 transition-colors"
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-gray-500 text-xs">
                                  {program.totalSessions} sessions · {program.sessionDuration}hr each
                                </p>
                              </div>
                            </div>
                      ...
                      {/* CTA */}
                      <Link
                        href={`/training/${academy.id}`}
                        className="block w-full mt-6 text-center border border-[#D4AF37]/40 text-[#D4AF37] hover:bg-[#D4AF37] hover:text-black transition-all duration-300 py-3.5 text-xs uppercase tracking-[0.2em] font-semibold rounded-full"
                      >
            ...
            <div className="container mx-auto px-4 max-w-5xl">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                {[
                  ...
                  <div
                    key={item.title}
                    className="text-center p-6 rounded-2xl border border-white/5 bg-white/[0.02] hover:border-[#D4AF37]/20 transition-colors duration-300"
                  >
        ...
        <section className="relative z-20 w-full bg-[#0a0a0a] py-32">
          <div className="container mx-auto px-4 max-w-lg text-center">
            ...
            <p className="text-gray-400 mb-8">
              Our training academies are being set up. Check back soon for
              professional horse riding courses across Egypt.
            </p>
            <Link
              href="/contact"
              className="inline-block border border-[#D4AF37]/50 text-[#D4AF37] hover:bg-white hover:text-black hover:border-white transition-colors duration-500 px-10 py-4 text-xs uppercase tracking-[0.2em] rounded-full"
            >
              Register Interest
            </Link>
          </div>
      ...
      <section className="relative z-20 w-full bg-gradient-to-b from-black via-[#050505] to-black py-24">
        <div className="container mx-auto px-4 max-w-3xl text-center">
          ...
          <h2 className="font-display text-3xl md:text-5xl font-light text-white mb-6">
            Start Your{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#D4AF37] to-[#F5E6B8]">
              Journey
            </span>
          </h2>

          <p className="text-gray-400 max-w-xl mx-auto mb-10">
            From first-time riders to advanced equestrians, our certified captains
            will guide you every step of the way.
          </p>

          <Link
            href="/contact"
            className="inline-block bg-white text-black px-10 py-4 text-xs uppercase tracking-[0.2em] font-semibold rounded-full hover:scale-105 transition-transform duration-300"
          >
            Contact Us
          </Link>
        </div>
