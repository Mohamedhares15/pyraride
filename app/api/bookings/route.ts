  ...
  try {
    ...
    const createdBookings = await prisma.$transaction(async (tx) => {
      ...
      for (const bookingItem of bookings) {
        ...
        // In our memory-based architecture we no longer need to physically reserve AvailabilitySlots.
        // The existence of the booking itself mathematically subtracts from stable availability.
