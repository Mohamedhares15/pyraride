...
export async function POST(req: NextRequest) {
  try {
    ...
    // New Feature Queries
    const isPackageQuery = /package|vip|luxury|exclusive|group|event|tour/i.test(userMessage);
    const isTransportQuery = /transport|pickup|car|drive|location|zone|transfer/i.test(userMessage);
...
${userRole === "rider" ? "ماذا تريد أن تفعل اليوم؟" : "كيف يمكنني مساعدتك؟"}`;
      ...
      if (activePackages.length > 0) {
        const packageList = activePackages.map((pkg: any) => 
          `👑 **${pkg.title}** (${pkg.packageType === "PRIVATE" ? "Private VIP" : "Group Event"})\n   ${pkg.duration} hrs • EGP ${pkg.price}\n   ${pkg.hasHorseRide ? "🐎 Ride " : ""}${pkg.hasFood ? "🍽️ Dinner " : ""}${pkg.hasDancingShow ? "💃 Show" : ""}`
        ).join("\n\n");

        response = `✨ **The Royal Collection**\n\nWe offer exclusive, curated experiences at the Great Pyramids built for the most discerning travelers. \n\n**Available Luxury Packages:**\n\n${packageList}\n\nWould you like to explore these experiences in detail?`;
        suggestions = ["View Packages", "How do I book a VIP package?", "Contact Support"];
        actions = { "Explore Royal Collection": "/packages" };
      } else {
        response = `✨ **The Royal Collection**\n\nOur experts are currently crafting new exclusive journeys and VIP experiences! Please check back soon or contact support for VIP requests.`;
        suggestions = ["Show me stables", "Contact Support"];
        actions = { "Browse Standard Rides": "/stables" };
      }
    }
      ...
      if (zones.length > 0) {
        const zoneList = zones.map((z: any) => 
          `🚗 **${z.name}**\n   Additional EGP ${z.price}`
        ).join("\n");

        response = `🚗 **Chauffeur & Pickup Services**\n\nWe offer private, comfortable transportation directly from your hotel or home to the Pyramids! You can easily add transportation during your package checkout.\n\n**Currently Available Zones:**\n\n${zoneList}\n\n*(If your area isn't listed, you can select "Meet at location" and arrange your own transport!)*`;
        suggestions = ["View Packages", "Show me stables", "How do I book?"];
        actions = { "Browse Packages": "/packages" };
      } else {
        response = `🚗 **Transportation Services**\n\nWe recommend arranging an Uber or Careem to reach the Pyramids. Our verified stables will provide you with their exact meeting points upon booking!`;
      }
    }
      ...
      if (minPrice !== null || maxPrice !== null) {
        ...
        if (horsesInRange.length > 0) {
          ...
          suggestions = ["Show all horses", "Show stables", "Find horses under EGP 60"];
        ...
        if (!ownerStable) {
          ...
          if (isPricingOptimizationQuery || isRevenueQuery) {
            const priceOptimization = bestPerformer
              ? `💰 **PRICING OPTIMIZATION INSIGHTS:**\n\n**Your Current Performance:**\n- Average price: EGP ${avgPrice.toFixed(0)}/hour\n- Market average: EGP ${marketAvgPrice.toFixed(0)}/hour\n- Total revenue (90 days): EGP ${totalRevenue.toFixed(0)}\n\n**AI Recommendations:**\n`
              : "";

            const recommendations = [];
            if (bestPerformer) {
              if (bestPerformer.price < marketAvgPrice * 0.9) {
                recommendations.push(`🚀 **${bestPerformer.name}** could increase price by 15% (EGP ${bestPerformer.price} → EGP ${Math.round(bestPerformer.price * 1.15)}) - Still below market average!\n   *Expected impact: +EGP ${Math.round(bestPerformer.revenue * 0.15)}/month*`);
              }
            }
