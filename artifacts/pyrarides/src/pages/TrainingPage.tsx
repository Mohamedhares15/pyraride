    ...
    <>
      <section ref={heroRef} className="relative h-[88svh] min-h-[640px] overflow-hidden">
        <motion.div style={{ y, scale }} className="absolute inset-0">
          <img src={arena} alt="Private dressage arena at golden hour with rider on grey Arabian horse" className="h-full w-full object-cover" />
      ...
      <section className="container">
        ...
        <StaggerGroup className="grid md:grid-cols-3 gap-px bg-hairline border hairline" gap={0.1}>
          {PROGRAMMES.map((p) => (
              ...
              <article className={cn(
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    p.featured ? "bg-background text-foreground hover:bg-background/90" : "bg-foreground text-background hover:bg-foreground/90",
                  )}
                >
                  Reserve programme
        ...
        <div className="grid lg:grid-cols-12 gap-12 items-center">
          ...
          <Reveal className="lg:col-span-7 lg:pl-8" delay={0.15}>
            ...
            <dl className="mt-12 grid grid-cols-3 gap-6 border-t hairline pt-8">
              {[["1982", "First saddle"], ["1996", "Atlanta Olympics"], ["2009", "Founded the school"]].map(([k, v]) => (
                <div key={k}>
        ...
        <div className="md:col-span-8">
          <StaggerGroup gap={0.06}>
            {FAQ_ITEMS.map((item, i) => (
              <StaggerItem key={item.q}><FaqRow item={item} defaultOpen={i === 0} /></StaggerItem>
            ))}
          </StaggerGroup>
        </div>
      </section>
