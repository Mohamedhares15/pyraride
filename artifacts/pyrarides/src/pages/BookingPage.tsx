...
const MOCK_LEDGER: Record<string, Record<string, { am: number; pm: number }>> = {
  h1: { [dateKey(new Date())]: { am: 2, pm: 0 } },
  h6: { [dateKey(new Date())]: { am: 1, pm: 1 } },
};
...
const BookingPage = () => {
  ...
  return (
    <div className="container pt-32 pb-32 min-h-screen">
        ...
        <div className="lg:col-span-8 min-h-[460px] relative">
          <AnimatePresence mode="wait" custom={direction}>
            ...
            >
              ...
              {step === 2 && sel.date && (
                <SlotPicker date={sel.date} horses={stableHorses} value={sel.slot} onSelect={(s) => setSel({ ...sel, slot: s, horseId: undefined })} />
              )}
                    ...
                    return (
                      <button key={h.id} onClick={onPick} className={cn("group text-left relative overflow-hidden transition-all duration-500", active ? "ring-1 ring-foreground" : "ring-1 ring-transparent hover:ring-hairline")}>
                        <div className="aspect-[3/4] overflow-hidden bg-surface relative">
                          <motion.img src={h.image} alt={h.name} className={cn("h-full w-full object-cover transition-[filter] duration-700", (!allowed || !slotOk) && "grayscale opacity-60")} whileHover={{ scale: 1.05 }} transition={{ duration: 1.2, ease: easeLuxury }} />
                          {(!allowed || !slotOk) && (
                            <span className="absolute inset-x-3 top-3 inline-flex items-center justify-between gap-2 bg-foreground/90 text-background px-3 py-1.5 text-[10px] tracking-luxury uppercase">
                              <span className="inline-flex items-center gap-1.5"><Lock className="size-3" />{!slotOk ? "Resting this slot" : `${h.adminTier} tier`}</span>
        ...
        <aside className="lg:col-span-4">
          <div className="lg:sticky lg:top-28 border hairline bg-surface-elevated/60 p-7">
            ...
            <div className="mt-6 pt-5 border-t hairline space-y-2 text-sm">
              <PriceLine label={horse ? `Horse · ${sel.party}h × $${horse.pricePerHour}` : "Subtotal"} value={subtotal ? `$${subtotal.toFixed(0)}` : "—"} />
              <PriceLine label="Concierge (8%)" value={subtotal ? `$${concierge.toFixed(0)}` : "—"} />
            </div>
      ...
      <LuxuryModal open={!!overrideHorse} onClose={() => setOverrideHorse(null)} eyebrow="Skill mismatch" title="A quiet word from the master rider.">
        {overrideHorse && (() => {
          ...
          return (
            <>
              <p className="text-ink-soft text-pretty leading-relaxed">
                ...
                Your current standing is <span className="text-foreground">{currentUser.rankPoints} pts</span>. You may request an override — our captain will personally review within the hour.
              </p>
...
const SlotPicker = ({ date, horses: roster, value, onSelect }: { date: Date; horses: Horse[]; value?: Slot; onSelect: (s: Slot) => void }) => {
  const availableCount = (slot: Slot) => roster.filter((h) => horseSlotAvailable(h.id, date, slot)).length;
  const Block = ({ slot }: { slot: Slot }) => {
    const count = availableCount(slot);
    const disabled = count === 0;
    const active = value === slot;
    return (
      <button onClick={() => !disabled && onSelect(slot)} disabled={disabled} className={cn("relative px-5 py-6 border hairline text-left transition-all duration-500", active ? "bg-foreground text-background border-foreground" : "hover:bg-surface/60", disabled && "opacity-30 cursor-not-allowed")}>
  ...
  return (
    <div className="space-y-10">
      <div>
        <p className="text-[10px] tracking-luxury uppercase text-ink-muted mb-4 inline-flex items-center gap-2"><Sun className="size-3.5" /> Morning · max 2 rides per horse</p>
        <div className="grid grid-cols-3 gap-3">{AM_SLOTS.map((s) => <Block key={s} slot={s} />)}</div>
      </div>
      <div>
        <p className="text-[10px] tracking-luxury uppercase text-ink-muted mb-4 inline-flex items-center gap-2"><Moon className="size-3.5" /> Afternoon & evening · max 1 ride per horse</p>
        <div className="grid grid-cols-3 gap-3">{PM_SLOTS.map((s) => <Block key={s} slot={s} />)}</div>
      </div>
      <p className="text-xs text-ink-muted">Slot capacity is computed per individual horse. Once you choose a time, only the horses with remaining welfare capacity for that hour will appear.</p>
    </div>
  );
};
